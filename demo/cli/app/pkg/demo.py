...
class PlatformDataUnit:
    ...
    def create_items(
        self, item_count: int, item_diff: str, category_path: str, do_publish: bool
    ):
        if not self.store_id:
        ...
        for i in range(item_count):
            ...
            for j in range(reward_item_count):
                ...
                for item_info in items:
                    ...
                    reward_box_items.append(
                        platform_models.BoxItem.create(
                            count=1, item_id=item_info.id, item_sku=item_info.sku
                        )
                    )
            ...
            new_item, error = platform_service.create_item(
                ...
                body=platform_models.ItemCreate.create(
                    ...
                    localizations={
                        "en": platform_models.Localization.create(
                            title=lootbox_item.title
                        )
                    },
                    region_data={
                        "US": [
    ...
    def consume_item_entitlement(self, user_id: str, entitlement_id: str, count: int):
        result, error = platform_service.consume_user_entitlement(
            ...
            body=platform_models.AdminEntitlementDecrement.create(
                use_count=count, request_id=random_string(ALPHA_CHARS, 8)
            ),
        )
        if error:
            return None, error
        ...
        for it in result.rewards:
            items.append(SimpleItemInfo(id=it.item_id, sku=it.item_sku))
